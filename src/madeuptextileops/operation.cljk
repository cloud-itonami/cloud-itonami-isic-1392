(ns madeuptextileops.operation
  "MadeUpTextileOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (MadeUpTextileAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Made-Up Textile Articles Operations Governor (:govern) and the
  ...
  each operation is auditable and checkpointed. A made-up-textile-
  articles plant's lifecycle is advanced by MANY ops (production-batch
  logging / maintenance scheduling / shipment coordination), each
  independent.
  ...
  (:require [langgraph.graph :as g]
            ...
            [madeuptextileops.advisor :as advisor]
            [madeuptextileops.governor :as governor]
            [madeuptextileops.phase :as phase]
...
(defn build
  "Compiles a MadeUpTextileOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `madeuptextileops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MadeUpTextileAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Made-Up Textile Articles Operations Governor -- independent censor.
      (g/add-node :govern
