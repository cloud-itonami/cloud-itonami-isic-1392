(ns madeuptextileops.store
  "SSoT for the made-up-textile-articles plant-operations coordination
  ...
  external made-up-textile-articles capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             cutting/sewing/hemming/quilting output
                             (bedding, curtains, towels, table linens):
                             quality-grade/finished-unit-count/defect-
                             ...
                             marks whether it is on file in the plant's
                             production ledger; `:shipped-units` tracks
                             the batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a cutting-line/sewing-line/serger-line/
                             quilting-line unit's own record.
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled cutting/sewing/hemming/
                             quilting-line maintenance-window DRAFT
                             against a piece of equipment
                             (`madeuptextileops.registry`'s
                             `register-maintenance`). Dedicated
                             `:scheduled?` double-schedule guard (never
                             a `:status` value).
    - `shipments`         -- a proposed outbound finished-goods
                             shipment DRAFT
                             (`madeuptextileops.registry`'s
                             `register-shipment`).
  ...
  unit, which shipment was coordinated and at what independently-
  recomputed unit count, approved by whom, which safety concern was
  flagged' is always a query over an immutable log -- the audit trail
  a plant owner or downstream buyer trusting this coordinator needs."
  (:require [madeuptextileops.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (madeuptextileops.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (madeuptextileops.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :process-type :sewing :product-type :bedding
                :quality-grade :grade-a
                :units-produced 20000.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-units 5000.0
                :last-inspected "2026-06-01"}
   "batch-002" {:id "batch-002" :process-type :cutting :product-type :curtains
                :quality-grade :grade-b
                :units-produced 6000.0 :defect-rate-percent 3.5
                :verified? true :registered? true
                :shipped-units 5700.0
                :last-inspected "2026-06-01"}
   "batch-003" {:id "batch-003" :process-type :hemming :product-type :towels
                :quality-grade :grade-c
                :units-produced 15000.0 :defect-rate-percent 8.0
                :verified? false :registered? false
                :shipped-units 0.0
                :last-inspected "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :cutting-line
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :sewing-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `madeuptextileops.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `madeuptextileops.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defrecord MemStore [a]
  ...
  (commit-record! [s {:keys [effect path value] :as record}]
    (cond
      ...
      ;; above (also what `finishingops`-style siblings expose as their
...
(defn sample-data!
  ...
  cutting-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered sewing-line unit (blocks any maintenance
