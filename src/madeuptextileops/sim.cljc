(ns madeuptextileops.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean made-up-textile-
  articles plant through intake -> maintenance scheduling (escalate/
  ...
  unrecognized op, maintenance scheduled against an UNVERIFIED/
  unregistered equipment unit, a shipment coordinated against an
  UNVERIFIED/unregistered batch, a shipment proposal that would exceed
  the batch's own logged finished-unit count, a proposal that tries to
  directly operate a cutting/sewing/hemming/quilting line (permanently
  blocked, no override), a double-schedule of the same maintenance
  window, a production-batch patch with a fabricated quality grade,
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, cutting-line -- escalates, approve) ==")
    ...
    (println "== coordinate-shipment ship-1 on batch-001 (verified, registered, within units -- escalates, approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-cutting-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered sewing line -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (1000 units would exceed 6000 vs shipped 5700 -> HARD hold) ==")
