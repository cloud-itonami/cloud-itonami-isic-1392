(ns madeuptextileops.governor
  "Made-Up Textile Articles Operations Governor -- the independent
  compliance layer that earns the MadeUpTextileAdvisor the right to
  ...
  merely draft-schedule) a cutting/sewing/hemming/quilting line,
  whether a shipment proposal's own claimed unit count would blow
  through the batch's own logged finished-unit count, or when an act
  stops being a coordination proposal and becomes direct cutting/
  sewing-line-equipment control, so this MUST be a separate system
  able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:made-up-textile-articles-operations-governor` (see docs/adr/0001-
  architecture.md).
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:cutting-line/
                                       actuate`) is the 'direct
                                       cutting/sewing-line-equipment
    ...
    4. Line-operate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       Directly operating a cutting/
                                       sewing/hemming/quilting line
   ...
   10. Invalid defect rate         -- for `:log-production-batch`, if
                                       ...
                                       let fabricated/inspection-error
                                       data through.
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct cutting/sewing/hemming/quilting-line-equipment
  control, a hallucinated actuation effect) is this actor's central
  scope boundary."
  [proposal]
...
(defn- line-operate-blocked-violations
  ...
  directly operate a cutting/sewing/hemming/quilting line -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :line-operate-blocked
      :detail "裁断・縫製ライン設備の直接操作提案は恒久的に禁止 -- 保守作業予定(draft)のみ許可"}]))
...
(defn- shipment-units-exceeded-violations
  ...
  batch's own recorded shipped-to-date units plus the proposal's own
  claimed units would exceed the batch's own recorded
  `:units-produced` -- ground truth from the batch's own permanent
  ...
  (when (= op :coordinate-shipment)
    (let [{:keys [batch-id units]} (:value proposal)
      ...
      (cond
        ...
        (not (registry/shipment-units-exceeded-checkable? b units))
        [{:rule :shipment-units-exceeded
          :detail "生産量/既存出荷実績/申請量のいずれかが数値として確定できない -- 空き容量を検算できないため出荷しない"}]

        (registry/shipment-units-exceeded? b units)
        [{:rule :shipment-units-exceeded
          :detail (str batch-id " の記録済み生産数量(" (:units-produced b)
...
(defn check
  "Censors a MadeUpTextileAdvisor proposal against the governor rules.
