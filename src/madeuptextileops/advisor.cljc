(ns madeuptextileops.advisor
  "MadeUpTextileAdvisor -- the *contained intelligence node* for the
  made-up-textile-articles plant-operations coordination actor
  (cutting, sewing, hemming, quilting lines producing bedding,
  curtains, towels, and table linens from finished fabric).

  It normalizes production-batch patches (quality-grade/finished-unit-
  count/defect-rate), drafts a cutting/sewing/hemming/quilting-line
  maintenance scheduling proposal against a piece of equipment, drafts
  an equipment-safety/quality-defect concern flag, and drafts an
  outbound finished-goods shipment coordination proposal against a
  ...
  a committed record and NEVER a real cutting/sewing-line-equipment
  actuation or a freight dispatch. Every output is censored downstream
  by `madeuptextileops.governor` before anything touches the SSoT --
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `madeuptextileops.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; sewing-line-equipment-control effect
  ...
  `madeuptextileops.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [madeuptextileops.registry :as registry]
...
(defn- log-production-batch
  ...
  finished-unit-count or defect-rate. High confidence, low stakes --
...
(defn- schedule-maintenance
  "Draft a cutting/sewing/hemming/quilting-line maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `madeuptextileops.governor` NEVER trusts this report
...
(defn- flag-safety-concern
  ...
  `madeuptextileops.phase`: no phase ever adds this op to a phase's
  `:auto` set; `madeuptextileops.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound finished-goods shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed unit count -- it does NOT invent one, and
  `madeuptextileops.governor` NEVER trusts it: it independently
  recomputes whether the batch's own cumulative-shipped units plus
  this claim would exceed the batch's own recorded finished-unit count
...
(def ^:private system-prompt
  (str "あなたは既製繊維製品(寝具・カーテン・タオル・テーブルリネン)"
       "縫製工場プラント運用コーディネーターの助言者です。"
       ...
       "裁断・縫製ラインの設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `madeuptextileops.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :made-up-textile-advisor-proposal
   :op         (:op request)
