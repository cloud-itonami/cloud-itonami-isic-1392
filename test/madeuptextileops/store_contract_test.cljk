(ns madeuptextileops.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `madeuptextileops.store` ns docstring for why a second (Datomic-
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:quality-grade :grade-b}})
    (is (= :grade-b (:quality-grade (store/batch s "batch-001"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-units
  (let [s (seeded)]
    ...
    (is (= 10000.0 (:shipped-units (store/batch s "batch-001")))
