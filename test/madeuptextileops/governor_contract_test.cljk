(ns madeuptextileops.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control cutting/sewing/hemming/quilting-
  line equipment directly... does NOT authorize or execute equipment
  operations') implemented faithfully. The single invariant under
  test:

    MadeUpTextileAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Made-Up Textile Articles
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-cutting-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest coordinate-shipment-always-needs-approval
  (testing "a CLEAN shipment coordination is never auto-eligible -- always escalates, even below any units threshold"
    (let [[db actor] (fresh)
