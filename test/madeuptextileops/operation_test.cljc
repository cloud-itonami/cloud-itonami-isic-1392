(ns madeuptextileops.operation-test
  "Smoke tests for the compiled MadeUpTextileOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `madeuptextileops.governor-contract-test`; the Store contract in
  `madeuptextileops.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MadeUpTextileOperationActor can be built with a store"
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :concern-type :equipment-safety
                                   :severity :moderate :description "cutting blade guard worn"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))

(deftest test-shipment-coordination-proposal
  (testing "Shipment coordination proposal is submitted and (when within units) escalates for approval"
    (let [s (-> (store/mem-store) (store/sample-data!))
          actor (op/build s)
