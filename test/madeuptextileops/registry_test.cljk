(ns madeuptextileops.registry-test
  (:require [clojure.test :refer [deftest is]]
            [madeuptextileops.registry :as r]))
...
(deftest small-shipment-within-units-does-not-exceed
  (is (false? (r/shipment-units-exceeded?
               {:units-produced 20000.0 :shipped-units 5000.0} 5000.0))))

(deftest shipment-that-pushes-past-units-exceeds
  (is (true? (r/shipment-units-exceeded?
              {:units-produced 6000.0 :shipped-units 5700.0} 1000.0))))

(deftest shipment-exactly-at-units-does-not-exceed
  (is (false? (r/shipment-units-exceeded?
               {:units-produced 6000.0 :shipped-units 5700.0} 300.0))
      "exactly at units-produced is not over, only strictly beyond"))

(deftest missing-units-is-not-flagged-exceeded
  (is (false? (r/shipment-units-exceeded? {} 100.0)))
  (is (false? (r/shipment-units-exceeded? {:units-produced 800.0} nil))))

;; ----------------------------- grade-valid? -----------------------------

(deftest known-grades-are-valid
  (doseq [g [:grade-a :grade-b :grade-c :reject]]
    (is (r/grade-valid? g))))
